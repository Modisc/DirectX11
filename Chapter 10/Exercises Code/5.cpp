...
// 
// 연습문제 10장 5번
// 기존에 주어진 깊이ㆍ스텐실 설정을 문제에서 주어진 설정으로 변경하여 적용하라.
// 
//
//***************************************************************************************
...
bool MirrorApp::Init()
{
	...
	HR(DirectX::CreateDDSTextureFromFile(md3dDevice,
		L"Textures/checkboard.dds", &texResource, &mFloorDiffuseMapSRV));
	ReleaseCOM(texResource); // view saves reference

	HR(DirectX::CreateDDSTextureFromFile(md3dDevice,
		L"Textures/brick01.dds", &texResource, &mWallDiffuseMapSRV));
	ReleaseCOM(texResource); // view saves reference

	HR(DirectX::CreateDDSTextureFromFile(md3dDevice,
		L"Textures/ice.dds", &texResource, &mMirrorDiffuseMapSRV));
	ReleaseCOM(texResource); // view saves reference
...
void MirrorApp::DrawScene()
{
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		//md3dImmediateContext->OMSetDepthStencilState(RenderStates::NonDepthForWallDSS, 0);		// *
		md3dImmediateContext->OMSetDepthStencilState(RenderStates::OnlyDepthForWallDSS, 0);		// *
		pass->Apply(0, md3dImmediateContext);
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		// 가시적인 거울 픽셀들을 스텐실 버퍼에 그린다. 이 시점에서는 거울의 깊이를 깊이 버퍼에 기록하지 않는다.
		// 그래야 반사된 두개골이 거울에 가려지지 않는다.
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		// 반사에 의해 정점들이 감기는 순서가 바뀌므로, 시계방향 삼각형들이
		// 선별되도록 설정한다.
		md3dImmediateContext->RSSetState(RenderStates::CullClockwiseRS);

		// ************************************************************************************************************************************
		// Only draw reflection into visible mirror pixels as marked by the stencil buffer. 
		// 스텐실 버퍼 항목이 1인 픽셀만 렌더링한다. 가시적인 거울 픽셀만 스텐실 버퍼 항목이 1이므로
		// 반사된 두개골은 거울의 가시적인 부분에만 렌더링 된다.
		// md3dImmediateContext->OMSetDepthStencilState(RenderStates::DrawReflectionDSS, 1);		*
		md3dImmediateContext->OMSetDepthStencilState(RenderStates::NonStencilForSkullDSS, 0);		// *
		pass->Apply(0, md3dImmediateContext);
		md3dImmediateContext->DrawIndexed(mSkullIndexCount, 0, 0);
	...
	// 거울을 통상적인 방식으로 후면 버퍼에 렌더링하되,
	// 반사상이 통과해 보이도록 투명도 혼합을 적용한다.
	// 
