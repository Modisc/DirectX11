	...
	mTheta(1.3f * MathHelper::Pi), mPhi(0.4f * MathHelper::Pi), mRadius(80.0f)
{
	...
	mWorldMat.Diffuse = XMFLOAT4(0.05f, 0.05f, 0.05f, 1.0f);
}
...
void BlendApp::DrawScene()
{
	...
	ID3DX11EffectTechnique* worldTech;

	worldTech = Effects::BasicFX->DepthLevelTech;
	...
	md3dImmediateContext->OMSetBlendState(RenderStates::DrawDepthComplexityBS, blendFactor, 0xffffffff);
	md3dImmediateContext->OMSetDepthStencilState(RenderStates::DepthDisableDSS, 0);

	worldTech->GetDesc(&techDesc);
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		Effects::BasicFX->SetMaterial(mWorldMat);

		md3dImmediateContext->RSSetState(RenderStates::NoCullRS);
		worldTech->GetPassByIndex(p)->Apply(0, md3dImmediateContext);
