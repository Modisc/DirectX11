...
// 
// 연습문제 10장 7번
// 9장 혼합 예제에 원기둥을 하나 그린 후, 제공되는 60 프레임짜리 번개 애니메이션을
// 가산 혼합을 이용해서 원기둥에 텍스처로 입혀라.
//
// 
//***************************************************************************************
...
BlendApp::BlendApp(HINSTANCE hInstance)
	: D3DApp(hInstance), mLandVB(0), mLandIB(0), mWavesVB(0), mWavesIB(0), mBoxVB(0), mBoxIB(0), mCylinderVB(0), mCylinderIB(0),
	mGrassMapSRV(0), mWavesMapSRV(0), mBoxMapSRV(0), mCylinderMapSRV(0), mWaterTexOffset(0.0f, 0.0f), mEyePosW(0.0f, 0.0f, 0.0f),
	mLandIndexCount(0),	mRenderOptions(RenderOptions::TexturesAndFog), mTheta(1.3f * MathHelper::Pi), mPhi(0.4f * MathHelper::Pi), mRadius(80.0f),
	mFileExtension(L".dds"), mFileName(L""), mFileNum(L""), mFrame(0), mAnimCount(0), mAnimPlayTime(4), mMaxImageFrame(60), mAnimFrameTime(0)
{
	...
	XMMATRIX cylinderOffset = XMMatrixTranslation(8.0f, 5.0f, -40.0f);
...
bool BlendApp::Init()
{
	...
	HR(DirectX::CreateDDSTextureFromFile(md3dDevice,
		L"BoltAnimDDS/Bolt001.dds", &texResource, &mCylinderMapSRV));
...
void BlendApp::UpdateScene(float dt)
{
	...
	if (mAnimCount < mMaxImageFrame)
	{
		PlayAnimation(dt);
	}
}

void BlendApp::PlayAnimation(float dt)
{
	ID3D11Resource* texResource = nullptr;
	wcscpy_s(mFileName, L"BoltAnimDDS/Bolt");
	mFrame += dt;
	if (mFrame >= mAnimFrameTime)
	{
		...
		HR(CreateDDSTextureFromFile(md3dDevice,
			mFileName, &texResource, &mCylinderMapSRV));
		mFrame = 0;
		ReleaseCOM(texResource);	// view saves reference
	}
}
...
void BlendApp::DrawScene()
{
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		md3dImmediateContext->OMSetDepthStencilState(RenderStates::NoDepthDSS, 0);

		cylinderTech->GetPassByIndex(p)->Apply(0, md3dImmediateContext);
		md3dImmediateContext->DrawIndexed(2520, 0, 0);
