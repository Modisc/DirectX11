...
void SolutionApp::BuildGeometryBuffers()
{
	...
	Vertex vertices[] =
	{
		...
		{ XMFLOAT3(+4.5f, +1.0f, 0.0f), XMFLOAT4((const float*)&Colors::Red)	}
	};

	D3D11_BUFFER_DESC vbd;
	vbd.Usage = D3D11_USAGE_IMMUTABLE;
	vbd.ByteWidth = sizeof(Vertex) * 9;
	...
	// 삼각형 목록을 그릴 때 색인의 순서가 중요한가?
	// => 정점의 감기 순서를 지켜서 전면, 후면을 맞춰주어야 한다.
	// => 즉, 색인의 순서를 신경써서 그려야 한다.
	UINT indices[] = {
		// 문제에서 원하는 것처럼 분리된 삼각형 3개를 그림
		0, 1, 2,
		3, 5, 4,
		6, 7, 8

		// 정점들을 모두 연결해서 삼각형 띠와 같은 모양으로 삼각형을 그림
		// 사용되는 색인의 개수가 21개이므로 색인 버퍼의 크기 및
		// 그리기에 사용되는 색인의 개수를 함께 변경해주어야 정상 작동한다.
