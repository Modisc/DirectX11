...
// 
//	연습문제 6장 10번
//	정점들의 메모리 소비량이 증가한 경우에는 128비트 색상 값을 32비트 색상 값으로
//	줄이는 것이 도움이 될 수 있다. 정점 구조체에서 128비트 색상 대신
//	32비트 색상을 사용하도록 상자 예제를 수정하라.
//	정점 구조체와 해당 입력 배치 서술 배열은 다음과 같은 모습이 되어야 할 것이다.
//	
//	struct Vertex
//	{
//		XMFLOAT3	Pos;
...
SolutionApp::SolutionApp(HINSTANCE hInstance)
	...
	mTheta(1.5f * MathHelper::Pi), mPhi(0.5f * MathHelper::Pi), mRadius(7.0f)
{
...
void SolutionApp::BuildGeometryBuffers()
{
	...
	Vertex vertices[] =
	{
		{ XMFLOAT3(-1.0f, -1.0f, -1.0f), Convert::ArgbToAbgr(XMCOLOR((const float*)&Colors::White).c)   }, // white
		{ XMFLOAT3(-1.0f, +1.0f, -1.0f), Convert::ArgbToAbgr(XMCOLOR((const float*)&Colors::Black).c)   },	// black
		{ XMFLOAT3(+1.0f, +1.0f, -1.0f), Convert::ArgbToAbgr(XMCOLOR((const float*)&Colors::Red).c)     }, // blue
		{ XMFLOAT3(+1.0f, -1.0f, -1.0f), Convert::ArgbToAbgr(XMCOLOR((const float*)&Colors::Green).c)   },	// green
		{ XMFLOAT3(-1.0f, -1.0f, +1.0f), Convert::ArgbToAbgr(XMCOLOR((const float*)&Colors::Blue).c)    },	// red
		{ XMFLOAT3(-1.0f, +1.0f, +1.0f), Convert::ArgbToAbgr(XMCOLOR((const float*)&Colors::Yellow).c)  }, // cyan
		{ XMFLOAT3(+1.0f, +1.0f, +1.0f), Convert::ArgbToAbgr(XMCOLOR((const float*)&Colors::Cyan).c)    },	// yellow
		{ XMFLOAT3(+1.0f, -1.0f, +1.0f), Convert::ArgbToAbgr(XMCOLOR((const float*)&Colors::Magenta).c) }	// magenta
	};
