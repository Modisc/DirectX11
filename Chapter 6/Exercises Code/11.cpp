...
// 
//	연습문제 6장 11번
//	뷰포트를 변경해서 해골이 출력 창의 한 부분사각형에만 렌더링되도록 두개골 예제를 수정하라.
//	(4.2.8절 129p 참고)
// 
// 
//***************************************************************************************
...
void SolutionApp::OnResize()
{
	...
	// D3DApp::OnResize() 함수에서 이미 뷰포트에 관련된 설정을 끝낸 후, SolutionApp::OnResize() 함수에서는
	// 투영 행렬에 대해서만 조작을 하게 설계가 되어있었는데, 연습문제 답안을 다른 코드는 건드리지 않고
	// 최대한 스크립트 하나에서 해결을 하기위해 비효율적이지만 이런 식으로, 다시 뷰포트를 재설정 하게 해줌.
	// 뷰포트 직사각형의 위치를 옮기고 싶다면 mScreenViewport.TopLeftX, mScreenViewport.TopLeftY 값을 변경해주면 된다.
