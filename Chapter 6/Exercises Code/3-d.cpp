...
void SolutionApp::BuildGeometryBuffers()
{
	...
	// 삼각형 띠를 그릴 때 색인의 순서가 중요한가?
	// => 색인의 순서에 따라 선이 연결돼서 자동으로 삼각형이 그려지는 것 같음
	// => 삼각형 정점들을 감기 순서에 맞춰 색인을 지정해야 하는가 테스트 했지만 그렇진 않음
	// => 짝수번째 삼각형과 홀수번째 삼각형의 정점 감기 순서가 다르기 때문에 선별 문제가 발생하지 않도록
	// => GPU 내부적으로 짝수 번째 삼각형의 처음 두 정점의 순서를 맞바꾸어서
	// => 홀수 번째 삼각형과 같은 순서가 되도록 함.
	UINT indices[] = {
		0, 1, 2,
		3, 4, 5,
		6, 7
