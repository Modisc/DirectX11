...
private:
	ID3D11Buffer* mLandVB;			// 지형의 정점 버퍼
	ID3D11Buffer* mLandIB;			// 지형의 색인 버퍼
	ID3D11Buffer* mWavesVB;			// 파도의 정점 버퍼
	ID3D11Buffer* mWavesIB;			// 파도의 색인 버퍼

	ID3DX11Effect* mFX;									// 효과 객체
	ID3DX11EffectTechnique* mTech;						// 효과 기법 객체
	ID3DX11EffectMatrixVariable* mfxWorldViewProj;		// 세계ㆍ시야ㆍ투영 결합 행렬

	ID3D11InputLayout* mInputLayout;					// 입력 배치

	ID3D11RasterizerState* mWireframeRS;				// 와이어프레임 래스터화기(?)
...
void WavesDemo::UpdateScene(float dt)
{
	...
	// 동적 정점 버퍼를 갱신할 때 Map 함수로 버퍼의 메모리 블록의
	// 시작을 가리키는 포인터를 얻어서 그 메모리 블록에 자료를 기록한다.

	D3D11_MAPPED_SUBRESOURCE mappedData;

	// 첫번째 인자 : 읽기ㆍ쓰기 접근을 원하는 자원을 가리키는 포인터
	// 두번째 인자 : 첫번째 인자의 자원에 담긴 부분자원의 색인(이 매개변수의 용도는 나중에 나온다. 현재는 부분자원을 사용하지 않으므로 0으로 지정)
	// 세번째 인자 : 자원들에 공통인 플래그(여기서는 하드웨어에게 현재 버퍼를 폐기하고 새 버퍼를 할당해서 그 포인터를 돌려달라고 지시)
	// 네번째 인자 : 책에서 사용하지 않는 추가적인 플래그
	// 다섯번째 인자 : D3D11_MAPPED_SUBRESOURCE 구조체를 가리키는 포인터를 돌려준다. 이를 통해서 버퍼에 자료를 기록하거나 읽으면 된다.
	HR(md3dImmediateContext->Map(mWavesVB, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedData));

	// D3D11_MAPPED_SUBRESOURCE::pData : 읽고 쓸 수 있는 자원의 생(raw) 메모리 블록을 가리키는 포인터
	// 자원에 담긴 자료에 맞는 형식으로 캐스팅 할 필요가 있다.
	// pData가 mWavesVB 메모리 블록을 가리키고 있으므로 pData의 값을 조작해서 mWavesVB의 내용을 변경한다.
	...
	// 버퍼를 다 갱신한 후에는 반드시 Unmap 메서드를 호출해야 한다.
...
void WavesDemo::BuildWavesGeometryBuffers()
{
	...
	// 정점 버퍼를 생성한다. 시뮬레이션의 모든 단계에서
	// 데이터를 업데이트하므로 공간만 할당한다.
	...
	// 색인 버퍼를 생성한다. 색인 버퍼는 고정되어 있으므로
	// 한 번만 생성하고 설정하면 된다.

	std::vector<UINT> indices(3*mWaves.TriangleCount()); // 3 indices per face(면 하나당 색인 3개)
