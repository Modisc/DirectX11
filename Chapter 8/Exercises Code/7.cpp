...
// 
// 연습문제 8장 7번
// 7장의 조명된 두개골 예제(LitSkull)의 지면, 기둥, 구에 텍스처를 입혀보라.
//
// 
//***************************************************************************************
#include "d3dApp.h"
...
private:
	...
	ID3D11ShaderResourceView* mBricksDiffuseMapSRV;
	ID3D11ShaderResourceView* mFloorDiffuseMapSRV;
	XMFLOAT4X4 mTextureTransform;
...
LitSkullApp::LitSkullApp(HINSTANCE hInstance)
	...
	mFloorDiffuseMapSRV(0), mBricksDiffuseMapSRV(0), mStoneDiffuseMapSRV(0)
{
	...
	XMStoreFloat4x4(&mTextureTransform, I);
...
LitSkullApp::~LitSkullApp()
{
	...
	ReleaseCOM(mSkullVB);
	ReleaseCOM(mSkullIB);
	ReleaseCOM(mBricksDiffuseMapSRV);
	ReleaseCOM(mFloorDiffuseMapSRV);
	ReleaseCOM(mStoneDiffuseMapSRV);
...
bool LitSkullApp::Init()
{
	...
	// InputLayouts가 셰이더 서명에 의존하므로 반드시 Effects를 먼저 초기화해야한다.
	...
	HR(CreateDDSTextureFromFile(md3dDevice, L"SkullTextures/stone.dds", &texResource, &mStoneDiffuseMapSRV));
	ReleaseCOM(texResource);
	HR(CreateDDSTextureFromFile(md3dDevice, L"SkullTextures/bricks.dds", &texResource, &mBricksDiffuseMapSRV));
	ReleaseCOM(texResource);
	HR(CreateDDSTextureFromFile(md3dDevice, L"SkullTextures/floor.dds", &texResource, &mFloorDiffuseMapSRV));	
	ReleaseCOM(texResource);
...
void LitSkullApp::DrawScene()
{
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		Effects::BasicFX->SetTexTransform(XMLoadFloat4x4(&mTextureTransform));
		...
		Effects::BasicFX->SetTexTransform(XMLoadFloat4x4(&mTextureTransform));
		Effects::BasicFX->SetDiffuseMap(mStoneDiffuseMapSRV);
		...
		for (int i = 0; i < 10; ++i)
		{
			...
			Effects::BasicFX->SetTexTransform(XMLoadFloat4x4(&mTextureTransform));
			Effects::BasicFX->SetDiffuseMap(mBricksDiffuseMapSRV);
