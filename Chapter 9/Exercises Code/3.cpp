...
// 
// 연습문제 9장 3번
// 지형보다 물을 먼저 그리도록 혼합 예제를 수정해서 결과를 살펴보고,
// 왜 그런 결과가 나왔는지 설명하라.
//
// 
//***************************************************************************************
	...
	mTheta(1.3f * MathHelper::Pi), mPhi(0.4f * MathHelper::Pi), mRadius(80.0f)
{
	...
	mLandMat.Diffuse = XMFLOAT4(1.0f, 1.0f, 1.0f, 0.5f); // 기존 alpha값은 1.0f
	...
	mBoxMat.Diffuse = XMFLOAT4(1.0f, 1.0f, 1.0f, 0.5f); // 기존 alpha값은 1.0f
...
void BlendApp::DrawScene()
{
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		// 지형보다 물을 먼저 그리면서, 물에서 혼합 상태를 설정해주면 물보다 먼저 그려진 박스는 물을 통과하여 보이지만
		// 이후에 그려진 지형은 물에 가려진 부분이 보이지 않는다.
		...
		// 위에서 혼합 상태를 설정하고 혼합 상태를 디폴트 값으로 돌려주지 않는다면 투명도 혼합이 계속 적용되므로
		// 뒤에 그려질 지형 또한 투명하게 변형된다. 먼저 그려진 박스와 물이 지형을 통과하여 지형 뒤에 가려져있어도 보이게 되는 것이다.
		...
		// 물을 그린 후 지형을 그리면서 혼합상태를 설정한다면 지형만 투명하게 변형된다.
		// 즉, 지형에 가려진 물이나 박스가 그대로 보이게 된다.
		// 대신, 물은 불투명하게 되므로 물에 가려진 박스의 나머지 부분은 보이지 않는다.
		// 또한, 투명도 혼합은 덧씌울 픽셀의 알파값을 이용하게 되는데, 지형의 경우 알파값이 1.0이어서
		// 그대로 투명도 혼합을 적용하게 되면 아무런 변화가 없다.
		// 따라서 알파값을 0.5로 조절하여 적용해보면 지형이 투명해지는 것을 알 수 있다.
		md3dImmediateContext->OMSetBlendState(RenderStates::TransparentBS, blendFactor, 0xffffffff);
		landAndWavesTech->GetPassByIndex(p)->Apply(0, md3dImmediateContext);
