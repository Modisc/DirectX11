...
// 
// 연습문제 9장 6번
// 적색 채널과 녹색 채널로의 색상 쓰기를 비활성화하는 혼합 상태를 생성해서 적용하도록
// 혼합 예제를 수정하라. 
//
// 
//***************************************************************************************
...
private:
	...
	// 연습문제 6번을 위한 ID3D11BlendState 변수 선언(장치에 적용할 혼합 상태 객체를 가리키는 포인터)
	ID3D11BlendState* mTransparentBSWithoutRG;
...
BlendApp::BlendApp(HINSTANCE hInstance)
	...
	mWaterTexOffset(0.0f, 0.0f), mEyePosW(0.0f, 0.0f, 0.0f), mLandIndexCount(0), mRenderOptions(RenderOptions::TexturesAndFog), mTransparentBSWithoutRG(0),
...
BlendApp::~BlendApp()
{
	...
	ReleaseCOM(mTransparentBSWithoutRG);
...
bool BlendApp::Init()
{
	...
	// 연습문제 9장 6번을 위한 혼합 상태 하나를 생성하고 설정함
	...
	HR(md3dDevice->CreateBlendState(&transparentDesc, &mTransparentBSWithoutRG));
	//
...
void BlendApp::DrawScene()
{
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		md3dImmediateContext->OMSetBlendState(mTransparentBSWithoutRG, blendFactor, 0xffffffff);
		landAndWavesTech->GetPassByIndex(p)->Apply(0, md3dImmediateContext);
