...
// 
// 연습문제 9장 1번
// 수면을 렌더링 하기 전에 첫 번째 표본의 추출을 비활성화 하는 코드가 수행되도록
// 이번 장의 혼합 예제를 수정하라.
// 
//
//***************************************************************************************
...
private:
	...
	// 연습문제 1번을 위한 ID3D11BlendState 변수 선언(장치에 적용할 혼합 상태 객체를 가리키는 포인터)
...
BlendApp::BlendApp(HINSTANCE hInstance)
	...
	mWaterTexOffset(0.0f, 0.0f), mEyePosW(0.0f, 0.0f, 0.0f), mLandIndexCount(0), mRenderOptions(RenderOptions::TexturesAndFog), mTransparentBS(0),
...
bool BlendApp::Init()
{
	...
	// 연습문제 9장 1번을 위한 혼합 상태 하나를 생성하고 설정함
...
void BlendApp::DrawScene()
{
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		// 기존 예제 코드의 아래 문장에서 0xffffffff의 마지막 f만 e로 변경해주면 원하는 결과가 실행된다.
		// 다만 지금은 문제 푸는 것 자체가 아니라 이것저것 해보는 것에 목적을 두고 있어서 혼합 상태를 직접
		// 생성, 설정하여 해결해봄.
		//md3dImmediateContext->OMSetBlendState(RenderStates::TransparentBS, blendFactor, 0xffffffff);
		md3dImmediateContext->OMSetBlendState(mTransparentBS, blendFactor, 0xfffffffe);
