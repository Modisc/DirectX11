...
// 
// 연습문제 7장 1번
// 이번 장의 조명 예제를, 표면의 광택도를 제어하는 반영 지수를 여러가지로
// 변경해서 실행해보라. 이를테면 p = 8, p = 32, p = 64, p = 128, p = 256, p = 512
// 등을 시도해볼것.
// 
//
//***************************************************************************************

// 총 세 개의 광원, 즉 지향광원, 점광원, 점적광원 각 하나를 동시에 활성화 한다.
// 지향광원은 고정된 상태로 유지, 점광원은 지형 주위 회전, 
// 점적광원은 카메라와 함께 움직이면서 카메라가 향한 방향으로 빛을 비춘다.
	...
	mInputLayout(0), mEyePosW(0.0f, 0.0f, 0.0f), mTheta(1.5f * MathHelper::Pi), mPhi(0.1f * MathHelper::Pi), mRadius(80.0f)
{
	...
	// 점적광원 구분이 편하도록 연습문제 1번 코드를 그대로 사용함.
	...
	// 표면의 광택도를 제어하는 반영 지수는 점적광원 구조체의 Spot 변수이다.
	/*mSpotLight.Spot = 8.0f;
	mSpotLight.Spot = 32.0f;
	mSpotLight.Spot = 64.0f;
	mSpotLight.Spot = 128.0f;
	mSpotLight.Spot = 256.0f;*/
...
void LightingApp::UpdateScene(float dt)
{
	...
	// 빛을 보낸다. 이에 의해, 마치 관찰자가 손전등을 들고 다니는 듯한
	// 효과를 준다.
...
void LightingApp::DrawScene()
{
	...
	// 시야행렬, 지형 머테리얼 설정 등 상수 버퍼 변수 값 설정해주는 부분이
	// 굳이 반복문 안에 들어가 있어야 하는 이유가 무엇인지 생각해보기.
...
void LightingApp::BuildVertexLayout()
{
	...
	// 정점 색상은 더이상 지정하지 않는다. 픽셀마다 조명 방정식을 적용해서 표면의 색을
	// 결정할 것이기 때문에, NORMAL값을 설정한다.
